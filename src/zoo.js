...
function entryCalculator(entrants) {
  if (entrants && Object.keys(entrants).length > 0) {
    ...
    return chaves.reduce(
      (valorTotal, pessoas) =>
        valorTotal + data.prices[pessoas] * entrants[pessoas],
      0
    );
  }
  return 0;
...
function oldestFromFirstSpecies(id) {
  const idAnimal = data.employees.find((empId) => empId.id === id)
    .responsibleFor[0];
...
const arrayDeAnimais = (infoId) => {
  const animaisId = data.employees.find((elem) => elem.id === infoId)
    .responsibleFor;
  let arrayNomes = [];
